...
(function(){
  ...
  function renderLiveStatus() {
    ...
    $('liveInputs').innerHTML=active.map(p=>{
      ...
      const detected=qualified&&!data.clipping&&!data.muted?C.noteName(data.pitch.midi):'—';
  ...
  function rebuildCharts(){state.charts.clear();state.judges.clear();for(const p of enabled()){const chart=C.makeChart(state.song,p,state.from,state.to);state.charts.set(p.id,chart);state.judges.set(p.id,new C.Judge(chart,{difficulty:state.difficulty,speed:state.speed,drums:p.type==='drums',mode:state.song.rhythmOnly?'rhythm':S.isAudio(p)?'exact':p.mode,verifiedHolds:S.isAudio(p)&&!state.song.rhythmOnly,onJudge:r=>feedback(p.id,r)}));}renderPads();}
  ...
  async function startSession(demo=false,loopRestart=false){
    ...
    try{
      ...
      if(!demo) {
        for(const p of enabled().filter(S.isAudio)){const error=S.chartError(state.charts.get(p.id).notes,p.id);if(error)throw Error(error);}
  ...
  function frame(stamp){
    ...
    if(state.status==='playing'){
      if(audio.ctx.state!=='running'){pauseGame('Audio was interrupted. Press Resume when ready.');t=state.position;}
        ...
        for(const p of enabled()){const j=state.judges.get(p.id);if(state.demo){let i=j.demoCursor||0;while(i<j.notes.length&&j.notes[i].time<=t){const n=j.notes[i++];j.hit(n.time,{lane:n.lane,pitch:n.pitch,token:`demo:${p.id}:${n.id}`});}j.demoCursor=i;}j.tick(t-(state.demo?0:(S.correctionMs(p,state.inputOffset)+(S.isAudio(p)?S.LOOKBACK_MS:0))/1000)*state.speed);}
