...
(function(){
  ...
  const hub=new StageMIDI.MIDIHub({onMessage:handleMIDI,onDevices:updateDevices,onDisconnect:id=>{if(state.status==='playing'&&players.some(p=>p.enabled&&!S.isAudio(p)&&(p.device===id||p.device==='any'))){pauseGame('An instrument disconnected. Reconnect it, then resume.');}releaseDevice(id);}});
  ...
  function persist(){storage.set('midi-stage-settings-v1',{players:players.map(({id,enabled,device,channel,mode,input,offsetMs,learned})=>({id,enabled,device,channel,mode,input,offsetMs,learned})),minVelocity:state.minVelocity,inputOffset:state.inputOffset,monitor:state.monitor});}
  ...
  async function startSession(demo=false,loopRestart=false){
    ...
    try{
      ...
      if(ticket!==state.startGeneration){audio.stop();live.stop();return;}
    ...
    }catch(e){state.status='ready';live.stop();audio.stop();setControls();toast(e.message||'The set could not start.');}finally{state.startLock=false;}
  }
  function pauseGame(message='Paused. Press Enter or Resume to continue.'){
    if(state.status!=='playing')return;state.position=C.clamp(audio.songAt(),state.from,state.to);state.status='paused';live.stop();audio.stop();
  ...
  function feedback(id,result){
    ...
    if(result.note&&['perfect','great','good'].includes(result.grade)){const hitLanes=result.note.lanes||[result.note.lane];for(const lane of hitLanes)flash(id,lane);if(!reduced)for(const lane of hitLanes.slice(0,4))for(let i=0;i<2;i++)state.particles.push({id,lane,created:time,vx:(i?1:-1)*22,vy:-60-i*18,color:state.charts.get(id).lanes[lane].color});}
  ...
  function finishSession(){
    if(state.status!=='playing')return;state.position=state.to;state.status='complete';live.stop();audio.stop();for(const j of state.judges.values())if(j.verifiedHolds)for(const n of [...j.activeHolds])j.finishHold(n,false);$('countdown').innerHTML='';
