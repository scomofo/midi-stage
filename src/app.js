...
(function(){
  ...
  function handleString(role,event) {
    ...
    const lane=j.lanes.findIndex(l=>l.pc===C.pc(event.pitch));
  ...
  function rebuildCharts(){state.charts.clear();state.judges.clear();for(const p of enabled()){const chart=C.makeChart(state.song,p,state.from,state.to);state.charts.set(p.id,chart);state.judges.set(p.id,new C.Judge(chart,{difficulty:state.difficulty,speed:state.speed,drums:p.type==='drums',mode:S.isAudio(p)?'exact':p.mode,verifiedHolds:S.isAudio(p),onJudge:r=>feedback(p.id,r)}));}renderPads();}
  ...
  function render(t,now){
    ...
    active.forEach((p,pi)=>{
      ...
      const drawNote=n=>{
        ...
        if(pr>.65&&(p.mode==='exact'||S.isAudio(p))&&p.type!=='drums'&&active.length<3&&lw>22)text(C.noteName(n.pitch),pos.x,pos.y-10,7,color);
      };
