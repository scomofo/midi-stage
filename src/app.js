...
(function(){
  ...
  function bestKey(){return ['midi-stage-best-v3',state.song.id,state.difficulty,state.speed,enabled().map(p=>`${p.id}:${p.source}:${p.mode}:${p.input}`).join('|')].join('/');}
  ...
  async function startSession(demo=false,loopRestart=false){
    ...
    try{
      ...
      await audio.begin({song:state.song,players,speed:state.speed,seek:state.position,end:state.to,countIn:true,guide:$('guide').checked,demo:state.demo,metronome:$('metronome').checked,buffer:state.buffer,audioOffset:Number($('audioOffset').value)||0});
