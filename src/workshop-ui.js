...
(function(root){
  ...
  const hasTargets=p=>p.parts.some(part=>part.notes.length)||Object.values(p.chordHighways||{}).some(events=>events.length);
  ...
  <dialog id="workshopDialog" class="workshop-dialog" aria-labelledby="workshopTitle">
    ...
    <div class="ws-layout"><aside class="ws-inspector">
      ...
      <details id="wsChordEditor" class="ws-generator"><summary>Chord targets · review and correct</summary><p class="ws-small">Edit the chord that gameplay actually judges. MIDI chords also update their source notes. Estimated chords are separate from the rhythm markers.</p><label>Chord target<select id="wsChordList"><option value="">New chord</option></select></label><div class="ws-note-form"><label>Start (sec)<input id="wsChordTime" type="number" min="0" step="0.001" value="0"></label><label>Hold (sec)<input id="wsChordLength" type="number" min="0.08" step="0.01" value="1"></label><label>MIDI pitches<input id="wsChordPitches" value="60, 64, 67" placeholder="60, 64, 67"></label></div><p class="ws-small" id="wsChordNotes">C4 · E4 · G4</p><div class="ws-actions"><button id="wsChordSave" class="button secondary small">Save chord</button><button id="wsChordDelete" class="button ghost small" disabled>Delete chord</button></div></details>
  ...
  class Controller{
    ...
    bind(){
      ...
      $('wsClearPart').onclick=()=>{if(confirm(`Clear the ${LABELS[this.role]} highway? You can undo this.`))this.commit(W.clearPart(this.project,this.role));};
      $('wsChordList').onchange=()=>{const n=this.project.chordHighways?.[this.role]?.[Number($('wsChordList').value)];$('wsChordDelete').disabled=$('wsChordList').value==='';if(!n||$('wsChordList').value==='')return;$('wsChordTime').value=n.time;$('wsChordLength').value=n.duration;$('wsChordPitches').value=n.pitches.join(', ');$('wsChordNotes').textContent=n.pitches.map(C.noteName).join(' · ');};
      $('wsChordPitches').oninput=()=>{const pitches=$('wsChordPitches').value.split(/[\s,]+/).filter(Boolean).map(Number);$('wsChordNotes').textContent=pitches.every(p=>Number.isInteger(p)&&p>=0&&p<=127)?pitches.map(C.noteName).join(' · '):'Enter MIDI note numbers from 0 to 127.';};
      $('wsChordSave').onclick=()=>this.attempt(()=>{const index=$('wsChordList').value===''?null:Number($('wsChordList').value),pitches=$('wsChordPitches').value.split(/[\s,]+/).filter(Boolean).map(Number);this.commit(W.reviseChord(this.project,this.role,index,{time:Number($('wsChordTime').value),duration:Number($('wsChordLength').value),pitches}));this.status('Chord target updated. Gameplay uses these tones.');});
      $('wsChordDelete').onclick=()=>this.attempt(()=>{if($('wsChordList').value!=='')this.commit(W.reviseChord(this.project,this.role,Number($('wsChordList').value),null));});
      ...
      $('wsPlay').onclick=()=>this.task(async ticket=>{const p=W.validate({...this.project,title:$('wsName').value});if(!hasTargets(p))throw Error('Build a highway or add at least one note before playing.');this.stop();this.onPublish(p,this.buffer);if(this.dialog.open)this.dialog.close();});
    }
    attempt(action){try{action();}catch(e){this.status(e.message);}}
    discard(){return !this.dirty||!hasTargets(this.project)||confirm('Replace this working draft? Save or export it first to keep it.');}
    applyTiming(){
    ...
    async rebuildQuick(ticket,{grid=false}={}){
      ...
      if(hasTargets(this.project)&&W.serialize(this.project)!==this.lastAuto&&!confirm('Replace this edited arrangement with rhythm-only highways? Undo remains available.'))return;
    ...
    bindQuick(){
      ...
      $('wsQuickPlay').onclick=()=>this.task(async ticket=>{
        const p=W.validate(this.project);if(!hasTargets(p))throw Error('Tap a tempo and build a highway first.');
    ...
    render(){
      ...
      this.renderNotes();this.renderChords();this.draw();this.renderQuick();this.renderBusy();
    }
    renderChords(){const events=this.project.chordHighways?.[this.role]||[];$('wsChordEditor').hidden=!['keys','guitar'].includes(this.role);$('wsChordList').innerHTML='<option value="">New chord</option>'+events.map((n,i)=>`<option value="${i}">${n.time.toFixed(2)}s · ${esc(n.name)} · ${n.pitches.map(C.noteName).join(' / ')} · ${esc(n.source)}</option>`).join('');$('wsChordDelete').disabled=true;}
