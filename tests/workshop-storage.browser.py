...
with tempfile.TemporaryDirectory(prefix='midi-stage-profile-') as profile,sync_playwright() as p:
    ...
    page.locator('#openWorkshop').click();page.locator('#wsAuto').uncheck();page.locator('#wsFile').set_input_files(payload)
    ...
    page.locator('#openWorkshop').click();page.locator('#wsAdvanced').evaluate('(e)=>e.open=true');page.locator('[data-ws-load]').click();page.wait_for_function('!MIDIStage.getWorkshopSnapshot().busy')
    ...
    # New v2 rhythm semantics must survive real storage and a separate process.
    page.locator('#wsAuto').check();page.locator('#wsFile').set_input_files(str(ROOT/'tests/fixtures/quick-import-pulse.mp3'))
    page.wait_for_function('!MIDIStage.getWorkshopSnapshot().busy')
    rhythm=page.evaluate('MIDIStage.getWorkshopSnapshot().project');page.locator('#wsSave').click()
    page.wait_for_function('!MIDIStage.getWorkshopSnapshot().busy')
    check('real IndexedDB saves v2 rhythm matching and source MP3',page.evaluate('(key)=>new StageWorkshop.Library().get(key).then(r=>r.project.version===2 && r.project.matching==="rhythm" && r.blob instanceof Blob)',rhythm['id']))
    context.close()
    context=p.chromium.launch_persistent_context(profile,**opts);page=context.pages[0]
    page.on('pageerror',lambda e:errors.append(str(e)));page.on('dialog',lambda d:d.accept())
    page.goto((ROOT/'MIDI-Stage.html').as_uri(),wait_until='load')
    ...
    page.locator('#openWorkshop').click();page.locator('#wsAdvanced').evaluate('(e)=>e.open=true');page.locator('[data-ws-load]').click();page.wait_for_function('!MIDIStage.getWorkshopSnapshot().busy')
    restored=page.evaluate('MIDIStage.getWorkshopSnapshot().project')
    check('rhythm note times and matching survive edit/reopen without regeneration',restored==rhythm)
    page.locator('[data-ws-delete]').click();page.wait_for_function('!MIDIStage.getWorkshopSnapshot().busy')
    check('deleting rhythm library entry removes chart and audio together',page.evaluate('(key)=>new StageWorkshop.Library().get(key).then(r=>r===null)',rhythm['id']))
